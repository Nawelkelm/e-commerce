...
// TEST: Simple function to verify routing works
const testUpdateStatus = async (req, res) => {
  console.log('=== TEST FUNCTION CALLED ===');
  res.json({ message: 'Test route works!', params: req.params, body: req.body });
};
...
const uploadPaymentProof = async (req, res) => {
  try {
    ...
    if (!order.paymentMethod || !order.paymentMethod.toLowerCase().includes('transferencia')) {
      return res.status(400).json({ 
        message: 'Payment proof can only be uploaded for bank transfer orders' 
      });
    }
    ...
    res.json({ 
      message: 'Payment proof uploaded successfully. Your payment is pending verification.',
...
const approvePayment = async (req, res) => {
  try {
    ...
    if (!order.paymentProofUrl) {
      return res.status(400).json({ 
        message: 'No payment proof has been uploaded for this order' 
      });
    }
...
const rejectPayment = async (req, res) => {
  try {
    ...
    // Update order - clear proof and set status back
    await Order.update({
      paymentProofUrl: null,
      paymentProofUploadedAt: null,
      paymentStatus: 'unpaid',
      adminNotes: reason || 'Payment proof rejected by admin'
    }, {
      where: { id }
    });

    logger.info(`Payment rejected for order: ${id} by admin: ${req.user.id}. Reason: ${reason}`);
    ...
    // emailService.sendPaymentRejectionEmail(order.User.email, reason);
