  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-surface-600 dark:text-surface-400">Análisis de ventas y rendimiento de tu tienda</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        {stats.map((stat) => (
          <div
            key={stat.name}
            className="relative bg-white dark:bg-surface-800 pt-5 px-4 pb-12 sm:pt-6 sm:px-6 shadow rounded-lg overflow-hidden"
          >
            <dt>
              <div className="absolute bg-primary-500 rounded-md p-3">
                <stat.icon className="h-6 w-6 text-white" aria-hidden="true" />
              </div>
              <p className="ml-16 text-sm font-medium text-surface-500 dark:text-surface-400 truncate">{stat.name}</p>
            </dt>
            ...
            <div className="h-64 flex items-end space-x-2">
              {salesByMonth.map((item, index) => (
                <div key={index} className="flex-1 flex flex-col items-center group relative">
                  <div 
                    className="bg-primary-500 w-full rounded-t hover:bg-primary-600 transition-colors"
                    style={{ 
                      height: `${(item.sales / Math.max(...salesByMonth.map(d => d.sales))) * 200}px`,
                      minHeight: '20px'
                    }}
                  >
                    <div className="hidden group-hover:block absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 bg-surface-900 text-white text-xs rounded py-1 px-2 whitespace-nowrap">
                      ${item.sales.toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                    </div>
                  </div>
                  <span className="text-xs text-surface-500 dark:text-surface-400 mt-2">{item.month}</span>
                </div>
              ))}
            </div>
          ) : (
          ...
          {topProducts && topProducts.length > 0 ? (
            <div className="space-y-4">
              {topProducts.map((product, index) => (
                <div key={product.id} className="flex items-center justify-between p-3 hover:bg-surface-50 dark:bg-surface-900 rounded-lg transition-colors">
                  <div className="flex items-center flex-1">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-surface-500 dark:text-surface-400">{product.sales} unidades vendidas</p>
                    </div>
                  </div>
      ...
      <div className="mt-8 bg-white dark:bg-surface-800 shadow rounded-lg p-6">
        <h3 className="text-lg font-medium text-surface-900 dark:text-white mb-4">
          ...
          {summary.conversionRate > 0 && (
            <div className="p-4 bg-primary-50 rounded-lg">
              <div className="flex items-center">
                ...
                <h4 className="font-medium text-primary-900">Tasa de Conversión</h4>
              </div>
